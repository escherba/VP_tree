""" VP-tree data structure
    ...
    This software was written by Paul Harrison, and is released into
    the public domain.
"""
...
class _Node:
    ...
    def help_find(self, item, distances, heap, distance):
        ...
        for child in self.children:
            heapq.heappush(heap, (child.minimum_distance(new_distances),
                                  1, child, new_distances))
...
def _make_node(items, distance, max_children):
    ...
    for i in xrange(n_children):
        dl = distance_list[(i + 1) * (len(distance_list) - 1) // n_children]
        split_points.append(dl)

    for i in xrange(n_children):
        child_items = [item for item in items
                       if split_points[i] < item[1][-1] <= split_points[i + 1]]
...
class VP_tree:
    def __init__(self, items, distance, max_children=2):
        """ items        : list of items to make tree out of
            ...
            Using larger max_children will reduce the time needed to construct
            the tree, but may make queries less efficient.
        """
    ...
    def find(self, item):
        """ Return iterator yielding items in tree in order of distance
        from supplied item.
        """
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2:
        print 'Please supply the filename of your dictionary, \
    ...
    while True:
        ...
        for result in tree.find(query):
            print '% 6d comparisons later... % 5d %s'\
                % (comparison_count, result[1], result[0])
            comparison_count = 0
            n += 1
            if n >= 5:
                break
